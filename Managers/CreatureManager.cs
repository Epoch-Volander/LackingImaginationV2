...
public static class PrefabManager
{
    static PrefabManager()
    {
        ...
        if (!typeof(Heightmap.Biome).GetCustomAttributes(typeof(FlagsAttribute), false).Any())
        {
            ...
            harmony.Patch(
                AccessTools.Method(typeof(Heightmap.Biome).GetType(), "GetCustomAttributes",
                    new[] { typeof(Type), typeof(bool) }),
                postfix: new HarmonyMethod(AccessTools.DeclaredMethod(typeof(PrefabManager), nameof(BiomeIsFlags))));
        }
    }
